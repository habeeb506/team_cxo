...
- **Team Members**, **Business Teams**, **Permissions**, and **Tasks** are full CRUD management pages: create/edit/view/delete, search, filter, pagination, CSV template download/import/export (with append or replace modes, per-row validation errors, and duplicate prevention), and bulk delete. These live under **Quick Links**, alongside placeholder tiles for modules not built yet (Floor Leaders, Applications, ...). Team Members' table is trimmed to just Name, Location, Place/Floor, Shift, Support, and Time Slot (every other field is still captured in the create/edit form, just not shown in the list) — the form also carries a full career/development profile (Career Level, Prior/Firm/Overall Experience, Date Of Joining, Last Promotion, Time In Role, Backup Team Member, Coach, Primary/Secondary/Other Portfolio, Learning Hours, Business Chemistry, Certifications Planned, CE Baseline, Mobile#), added alongside the original fields; Over All/Firm Experience and Time In Role are read-only, computed automatically from Date Of Joining/Prior Experience/Last Promotion rather than typed in. Its CSV template covers every settable identity/org/profile field, none of Support/Shift/Time Slot (roster-driven, belongs to the separate Team Roster template instead — see the Team roster bullet below) and none of the three computed fields. It also has a roster snapshot bar above the table (Total Team plus Available/Training/Reconciliation/MFA/Dlaunch/PTO/ePTO/Other counts for a selectable day/week/month), an "Upload Monthly Roster" CSV control — see the Team roster bullet below — and a List/Schedule view toggle — see the Weekly schedule bullet below.
